...
| メニュー | 内容 |
|---|---|
| **1** | 動画を選択してOCRを実行。新シーズンはWikiから自動検索・登録。OCR後の欠損データを対話型で補完し、そのままGitHubへアップロードする流れに遷移可能。 |
| **2** | 既存のParquetファイルを選択し、欠損している順位のスコアを対話型で手動補完。完了後にGitHubへアップロード可能。 |
| **3** | `rank_data/` 内の変更されたデータをGitへコミットし、GitHubへプッシュ。 |
| **4** | 指定イベントIDのスコア・タイム分布グラフを画像として生成。 |
...
* **録画動画からのOCR（画像認識）解析方式**:
  ...
  4. 補完完了後、「GitHubへアップデートしますか？」のプロンプトに従い、そのままデータをアップロードできます。
