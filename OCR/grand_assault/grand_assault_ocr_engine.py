...
class TemplateParser:
    ...
    def parse_image(self, image_path, threshold_rank=0.84):
        ...
        if len(img.shape) == 2 or img.shape[2] == 1:
            ...
            crop_x_start = int(width * (740 / 1920))
            crop_x_end = int(width * (940 / 1920))
        ...
        score_rows = []
        x_start_score = 0  # Scan whole cropped region for Grand Assault score (no rank interference)
        x_end_score = gray.shape[1]
        ...
        for k in offsets:
            ...
            # Limit the search range specifically up to x_end_score (which corresponds to absolute X = 910)
            raw_scores = self.match_column(gray_score_roi, self.score_templates, 0, x_end_score - x_start_score, is_rank=False, threshold=0.78)
            ...
            if raw_scores:
                ...
                # Correction logic for Grand Assault:
                # 1. If the score is 10 digits, it means a trailing '1' was misrecognized from white background noise. Remove it to make it 9 digits.
                if len(score_digits) == 10:
                    score_digits = score_digits[:9]
                # 2. If the score is 9 digits and does not start with '1' (impossible as genuine 9-digit scores are >= 100M),
                # it means a trailing '1' was misrecognized. Remove it to restore the correct 8-digit score.
                if len(score_digits) == 9 and score_digits[0] != '1':
                    score_digits = score_digits[:8]
