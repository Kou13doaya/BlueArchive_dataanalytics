...
import numpy as np
import easyocr
from total_assault_ocr_engine import TemplateParser
...
def check_and_register_season(target_event_id):
    ...
    if base_event_id not in EVENT_META:
        ...
        if boss and period:
            ...
            success = register_new_event(base_event_id, season_str, boss, period, field=field)
            if success:
...
if __name__ == "__main__":
    ...
    if args.video:
        ...
        # 指定がない場合、インタラクティブ選択
        print("[INFO] video フォルダ内に以下の正しい形式の動画が見つかりました:")
